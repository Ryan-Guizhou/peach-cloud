...
- 禁止新代码使用字段 `@Resource` 或 `@Autowired`；实质修改 Bean 依赖或职责时统一该类构造器注入，并处理 qualifier、可选依赖与循环依赖。仅修改注释、文案或与依赖无关的局部逻辑不触发整类迁移，范围遵循 AGENTS.md。
...
仅在语义等价时采用，不全文件机械替换。检查集合可变性、null 元素/键值、空白字符语义及文件编码；`List.of/Map.of` 的 null 限制和 `Stream.toList()` 的不可修改性不得改变原行为。
...
- DO/持久化实体改 record
- 用虚拟线程替换 Redisson delay queue 监听线程、Quartz 调度线程或 CPU 密集型平台线程池
...
| 阻塞 IO 异步 | 使用 `peach-virtual-thread` 的分组执行器，按下游资源配置 `max-concurrency` 与 `max-pending` |
| 存量 `peach-threadpool` | 模块暂时保留以兼容旧代码；新增或实质修改代码不再引入 `ThreadPoolManager`、`@AsyncExecuted` 或 `PoolType.VIRTUAL` |
...
本次新建及依赖迁移的 Spring Bean 不应再使用字段 `@Resource` / `@Autowired`；未触及的存量不属于全仓验收范围。
