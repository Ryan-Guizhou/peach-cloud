...
- 涉及 Java/TypeScript 符号、调用链、starter 装配或模块运行机制时，先用 CodeGraph 建立事实，再读取模块 `pom.xml`、入口、配置类、配置元数据、自动配置、默认实现、quickstart 和现有 README。
...
- starter 文档必须区分 `autoconfigure`、`starter`、`quickstart` 三层职责，说明配置提示来源、条件装配、依赖传递和默认 Bean 覆盖方式。
