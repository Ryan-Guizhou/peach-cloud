...
- 新 Service 方法返回明确结果或领域结果，Controller 负责 Response 包装。存量 Response 签名在兼容维护时保留，不自动更改所有调用方，也不扩散到新内部服务。
- DTO 复用由语义决定：创建/更新字段和校验接近时沿用分组；授权、密码重置等不同命令使用独立 DTO，不用大 DTO 承载互不相关操作。
- 更新区分“不传”“显式清空”“设置值”，遵循当前接口契约；不靠全量 copyProperties 决定可写字段，不让省略字段意外覆盖库中值。
- 预期业务失败使用当前域的异常/结果体系；禁止捕获 Exception 后返回空集合、null 或成功来隐藏失败。确需降级时明确条件、返回语义和可观测证据；异常在负责处理的边界记录一次。
- 外部输入、可选结果及跨系统边界需要校验；内部已证明的必需依赖通过构造器和契约表达，不为每个内部参数机械重复判空。
- 客户端可提交租户/组织切换目标，服务端验证成员关系后生成最终上下文；查询和写入使用已验证的上下文，不能直接信任目标标识。
- 写操作检查受影响行数与业务状态；并发唯一性不能只靠先查再插，采用适合现有 schema 的约束或并发控制并验证冲突结果。
- 金额使用精确数值并明确舍入；时间语义和时区明确，存量字符串/数据库契约不在普通任务中自动迁移。批量流程注意循环 DAO/远程调用造成的负载放大。
