...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-pro-max`, `brand`, `design-system`, and browser tooling available in the current Cursor session.
...
Use Cursor TODO tracking and parallel subagents when available. Break down into:
...
1. **`brand`** — Extract brand colors, fonts, and voice from the user's project.
2. **`design-system`** — Get design tokens (spacing, typography scale, color palette).
3. **`ui-ux-pro-max`** — Apply layout, hierarchy, accessibility, and visual-balance guidance.
...
Use browser screenshot tooling, Playwright, or Puppeteer when exposed and already available to capture exact-size screenshots.
...
#### Option B: Cursor browser tooling

Use the browser or screenshot capability exposed by the current Cursor session to:
...
Use an available image-reading or browser capability to visually inspect each exported PNG:
...
Organize all output files and reports by campaign, platform, and size:
...
- AI image generation (only when the user explicitly requests it and an image-generation tool is available)
