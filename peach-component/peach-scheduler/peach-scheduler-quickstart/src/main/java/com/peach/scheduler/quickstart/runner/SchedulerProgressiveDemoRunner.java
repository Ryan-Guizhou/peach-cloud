...
/**
 * 按 basic → intermediate → advanced 顺序执行 Scheduler 演示。
 *
 * <p>可通过 {@code quickstart.scheduler.demo.levels} 裁剪级别。</p>
 *
 * @Author Mr Shu
 * @Version 1.0.0
...
public class SchedulerProgressiveDemoRunner implements ApplicationRunner {

    private final List<ProgressiveDemo> demos;
    private final Set<DemoLevel> enabledLevels;

    public SchedulerProgressiveDemoRunner(
            List<ProgressiveDemo> demos,
            @Value("${quickstart.scheduler.demo.levels:basic,intermediate,advanced}") String levels) {
        this.demos = demos;
    ...
    public void run(ApplicationArguments args) {
        ...
        log.info("scheduler progressive demo start, levels={}, demos={}", enabledLevels, ordered.size());
        for (ProgressiveDemo demo : ordered) {
            log.info(">>> running {}", demo.title());
            demo.execute();
            log.info("<<< finished {}", demo.title());
        }
        log.info("scheduler progressive demo finished");
    }
