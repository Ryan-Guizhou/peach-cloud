...
public class PeachCodeEvent {
    ...
    private final QuickstartCodeService quickstartCodeService;

    @Value("${peach.code.quickstart.verify-on-startup:true}")
    private boolean verifyOnStartup;

    @Value("${peach.code.quickstart.tenant-id:T001}")
    private String tenantId;

    /**
     * 创建启动验证事件。
     *
     * @param quickstartCodeService 示例业务服务
     */
    public PeachCodeEvent(QuickstartCodeService quickstartCodeService) {
        this.quickstartCodeService = quickstartCodeService;
    }
    ...
    public void verifyGeneratedCodes(ApplicationReadyEvent event) {
        if (!verifyOnStartup) {
            LOG.info("Code quickstart startup verification is disabled. tenantId={}", tenantId);
            return;
        }
        try {
            String menuCode = quickstartCodeService.createMenuCode(tenantId);
            String noticeCode = quickstartCodeService.createNoticeCode(tenantId);
            boolean menuValid = matchesPrefixAndNumber(menuCode, "MENU");
            boolean noticeValid = matchesPrefixAndNumber(noticeCode, "NOTICE");
            LOG.info("Code quickstart verification completed. tenantId={}, menuCode={}, menuValid={}, "
                            + "noticeCode={}, noticeValid={}",
                    tenantId, menuCode, menuValid, noticeCode, noticeValid);
        } catch (Exception ex) {
            LOG.error("Code quickstart verification failed. tenantId={}", tenantId, ex);
        }
    }
