package com.peach.initialize.impl.composite;
...
public class CompositeContainer<T>{
    ...
    public void init(ConfigurableApplicationContext context){
        Map<String, AbstractComposite> compositeInterfaceMap = context.getBeansOfType(AbstractComposite.class);
    ...
    public void execute(String type,T param){
        AbstractComposite<T> a = Optional.ofNullable(allCompositeInterfaceMap.get(type))
                .orElseThrow(() -> new RuntimeException("No corresponding component type found"));
        a.allExecute(param);
    }
