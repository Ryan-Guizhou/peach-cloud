...
public class FileFeignClientFallbackFactory implements FallbackFactory<FileFeignClient> {
    ...
    public FileFeignClient create(Throwable cause) {
        return new FallbackFileFeignClient(fallbackSupport, cause);
    }
    ...
    private static final class FallbackFileFeignClient implements FileFeignClient {

        private final PeachFeignFallbackSupport fallbackSupport;
        private final Throwable cause;

        private FallbackFileFeignClient(PeachFeignFallbackSupport fallbackSupport, Throwable cause) {
            this.fallbackSupport = fallbackSupport;
            this.cause = cause;
        }
